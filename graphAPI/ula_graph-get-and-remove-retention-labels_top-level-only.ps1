...
$script:nFolderLevel = 0
$script:nFolderLevelMax = 2  #-- How many folder levels are we going to recurse thru
...
function udf_AuthGraphApi
{
    ...
    $sClientId = "2417f530-7fe2-48ef-85c7-d5f09b9564d7"  #-- files.readwrite.all, sites,readwrite.all
    $sSecretB64 = "OWx4OFF+MGlrSnNabVVUYS1wNXZXfjVTS0hqVE9lY3ozX29BaGNqTQ==" #-- (expires 2/23/2025, ftan-sam-provisioning)
...
function udf_GetChildItems
{
    ...
    foreach ($oFolder in $rFolders)
    {
        ...
        $script:nFolderLevel++
        if ($script:nFolderLevel -le $script:nFolderLevelMax)
        {
...
{
    #-- We process in batches of 10, so this is the starting point, just increment it by 10
    $script:nIndex++
    #if ($script:nIndex -lt 70) {continue}
    #if ($script:nIndex -gt 191) {break}
